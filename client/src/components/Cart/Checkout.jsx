...
export const Checkout = () => {
    const { cart, total, updateQuantity, removeFromCart, clearCart, increment, decrement } = useCart();
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            clearCart();
            // opcional: redirigir a /orders/:id o pagina de confirmacion
                ...
                {cart.map((item) => (
                    <div key={`${item.type}-${item.refId}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #eee" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <button onClick={() => { removeFromCart(item.refId, item.type); successToast("Artículo eliminado"); }} style={{ marginLeft: 12 }}>Eliminar</button>
                        </div>
                    </div>
                ))}
            </div>
